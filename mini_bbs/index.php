...
//ログインしている状態でかつ、一時間何もしない状態でいるとログアウトする。
if(isset($_SESSION['id']) && $_SESSION['time'] + 3600 > time()){
  $_SESSION['time'] = time(); //sessionのtimeを都度更新してあげる
...
if(!empty($_POST)){
  if($_POST['message'] !== ''){
    ...
    $message->execute(array(
      $member['id'], //上のsessionのidより、DBから取ってきたidの方が確実なので。
      $_POST['message'],
      $_POST['reply_post_id']
    ));
    header('Location: index.php'); //裏で$_POSTの値を持ち続けているので、投稿した後に再読み込みするとDBに再度登録されてしまう。投稿したらindex.phpに飛ぶ処理をしてあげる。
    exit();
  }
}
//メッセージの取得
...
$posts = $db->prepare('SELECT m.name, m.picture, p.*FROM members m, posts p WHERE m.id=p.member_id ORDER BY p.created DESC LIMIT ?,5');//リレーションをはってる
